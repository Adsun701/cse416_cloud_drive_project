...
async function getFiles(searchQuery, token) {
  ...
  let searchString = queryArray[0]; // string
  ...
  for (let i = 0; i < operators.length; i += 1) {
    ...
    // case: op is equal to 'name'
    if (op === 'name') {
      if (val.length > 0) names.push(val);
    }
  }

  // check if any names were specified
  // eslint-disable-next-line prefer-destructuring
  if (names.length > 0) searchString = names[0];
  ...
  for (let i = 0; i < fromUsers.length; i += 1) {
    if (i === 0) fromUsersString += '(';
    fromUsersString += `('${fromUsers[i]}' in owners or '${fromUsers[i]}' in writers) and sharedWithMe = true`;
    if (i < fromUsers.length - 1) fromUsersString += ' or ';
    else fromUsersString += ')';
  }
  ...
  for (let i = 0; i < owners.length; i += 1) {
    if (i === 0) ownerString += '(';
    ownerString += `'${owners[i]}' in owners`;
    if (i < owners.length - 1) ownerString += ' or ';
    else ownerString += ')';
  }
  ...
  for (let i = 0; i < readers.length; i += 1) {
    if (i === 0) readerString += '(';
    readerString += `'${readers[i]}' in readers`;
    if (i < readers.length - 1) readerString += ' or ';
    else readerString += ')';
  }
  ...
  for (let i = 0; i < writers.length; i += 1) {
    if (i === 0) writerString += '(';
    writerString += `'${writers[i]}' in writers`;
    if (i < writers.length - 1) writerString += ' or ';
    else writerString += ')';
  }
...
router.post('/addaccesspolicy', async (req, res, next) => {
  try {
    ...
    User.update(
      { email },
      { $push: { accessPolicies: accessPolicy } },
    ).then(() => {});
...
router.post('/updateaccesspolicy/:requirement/ar', (req, res) => {
  ...
  AccessPolicy.update({ requirement }, { $push: { ar: newar } }).then(() => {
...
router.post('/updateaccesspolicy/:requirement/dr', (req, res) => {
  ...
  AccessPolicy.update({ requirement }, { $push: { dr: newdr } }).then(() => {
    const redirecturl = `/google/viewaccesspolicy?requirement=${requirement}`;
    res.redirect(redirecturl);
  });
});
...
router.post('/updateaccesspolicy/:requirement/aw', (req, res) => {
  ...
  AccessPolicy.update({ requirement }, { $push: { aw: newaw } }).then(() => {
    const redirecturl = `/google/viewaccesspolicy?requirement=${requirement}`;
    res.redirect(redirecturl);
  });
});
...
router.post('/updateaccesspolicy/:requirement/dw', (req, res) => {
  ...
  AccessPolicy.update({ requirement }, { $push: { dw: newdw } }).then(() => {
    const redirecturl = `/google/viewaccesspolicy?requirement=${requirement}`;
    res.redirect(redirecturl);
  });
});
...
router.get('/viewaccesspolicy', (req, res) => {
  ...
  AccessPolicy.findOne({ requirement }).then((data) => {
    ...
    const maxlength = Math.max(
      Math.max(Math.max(ar.length, dr.length), aw.length),
      dw.length,
    );
    const accessdata = [];
